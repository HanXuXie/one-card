...
try:
    ...
    _missing_module = str(e).replace("No module named ", "").strip("'")
...
class TerminalApp:
    def __init__(self, root):
        ...
        try:
            ...
            self._show_dependency_error(str(e))
            return
    ...
    def _show_dependency_error(self, msg=""):
        ...
        if msg:
            error_msg += f"\n{msg}"
        if not DEPENDENCIES_AVAILABLE:
            error_msg = f"缺少必要的依赖模块: {_missing_module}\n请先完成 Person B (card_manager.py) 和 Person C (calculator.py) 的实现"
        else:
            error_msg = f"初始化失败: {msg}\n请检查依赖模块是否正确安装"

        error_win = tk.Toplevel(self.root)
        error_win.title("启动错误")
        error_win.geometry("400x150")
        error_win.resizable(False, False)

        tk.Label(error_win, text=error_msg, wraplength=380,
                 font=('Arial', 11), pady=20).pack()
        tk.Button(error_win, text="确定", command=self.root.destroy,
    ...
    def _build_ui(self):
        ...
        if not self._has_dnd:
            self.select_btn = tk.Button(keypad, text='选择卡片', width=28, height=2,
                                        font=('Arial', 12), command=self._on_select_card)
            self.select_btn.grid(row=5, column=0, columnspan=4, padx=2, pady=5)
            self.select_btn.grid_remove()  # 初始隐藏
    ...
    def _on_select_card(self):
        ...
        filepath = filedialog.askopenfilename(
            title='选择卡片文件',
            initialdir=CARDS_DIR if os.path.exists(CARDS_DIR) else '.'
        )
        if filepath:
